...
export const createCommentService = async({userId, postId, content}:Props)
 :Promise<{comment: CommentDocument}> =>
    {
...
export const deleteCommentService = async({userId, commentId}:DeleteCommentInput ):Promise<{message:string}>=>{
            const user = await UserModel.findOne({ clerkId: userId });
            appAssert(user, NOT_FOUND, "User not found");
            ...
            appAssert(comment.user.toString() === user._id.toString(),
                    FORBIDDEN,
